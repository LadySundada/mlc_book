...
*概念：各种运动技能表现的具体特征为我们理解运动控制提供了基础。*

完成本章的学习后，你将能够：

描述菲兹定律并解释它与速度-准确性权衡现象的关系
定义*抓握*并举个例子，从运动控制的角度讨论抓握运动技能各组成部分之间的关系，并讨论视觉在抓握中的作用
描述书写如何为运动等效概念提供了一个很好的例子，以及视觉对书写的影响
描述对称和非对称双手协调的区别，并讨论非对称双手协调比对称双手协调更难学的原因
描述与走和跑步态模式相关的节奏关系，移动过程中保持头部稳定性的作用，以及与特定移动速度下发生的步态转换相关的特征。
描述接住移动物体的三个动作阶段以及视觉在每个阶段的作用，并回答是否有必要在接住飞行物体的整个过程中都要看见自己的手
论述视觉对击打移动物体的影响，以及这种影响透露出来的有关这类运动技能控制的信息
描述当运动目标为接触或躲避环境中的物体时，视觉是如何影响移动的

**应用**

适当的指导和练习干预程序对于帮助人们有效并高效地获取或恢复运动技能是很重要的。正如第5章和第6章所示，对运动控制理论和运动控制中的感觉过程的基本理解构成了制定这些程序的基础的重要组成部分。考虑下面两个例子，如果一个人在伸手、抓物和用杯子喝水时有困难，那么治疗师会如何制定合适的干预策略来帮助这个人完成这类技能呢？这个问题的答案主要来自对抓握的运动控制进行的研究。或者，假设网球班的一名新生在学习发球时遇到了问题，因为他/她无法协调抛球动作和挥拍动作，而这两个动作必须要同时发生才能发球成功。运动控制研究人员已经确定了一些明显的双手协调特征，可以帮助老师和教练员洞悉该如何克服网球发球问题。

这两个例子表明，理解与特定运动技能相关的运动控制过程和特征，可以帮助你洞悉该如何帮助人们克服在技能学习和康复背景中遇到的问题。我们可以考虑更多的例子，但这两个例子应该能够让你看到，如何将对运动技能的表现和运动控制特征应用于帮助客户实现学习或康复运动技能的诉求。在本章的讨论部分中，你将了解到与各种技能相关的具体表现和运动控制特征。其中一部分是特定运动技能所独有的，另一部分则是几项技能所共有的。如果这里面包含你在未来的职业生涯中希望帮助人们学习或康复的技能，那么你可以发现一些需要考虑进来以促进技能获取的具体表现和运动控制特征。如果没有包含的话，你可以在这些信息所提供的知识基础上，为那些你希望帮助人们学习或康复的技能建立具体的应用。

**讨论**

在第5章中讨论的运动控制理论和第6章中讨论的感觉信息的作用，都来自于研究人员对人们在不同情境下执行各种日常生活技能和体育运动技能的观察。这些观察结果表明，每种类型的运动技能都包含不同的表现特征，这些特征对于我们理解运动控制过程和为从业人员帮助人们学习或康复技能提供指导是必不可少的。接下来，我们将讨论几种比较重要的运动技能，介绍研究人员以此为目标的研究成果。这些技能没有特别的顺序，但却提供了各种类型的技能，从而明确如何将第5章和第6章中讨论的概念应用于特定的功能性运动技能。

需要指出的是，在这些技能的讨论中，我们主要考虑参与运动控制的感觉系统中视觉的作用。尽管第6章中提到，前庭、触觉和本体感觉的感觉信息在功能性技能的运动控制中也起作用，但对这些作用的讨论超出了本书的范围。

**速度-准确性技能**

许多运动技能都要求执行者既要有速度又要有准确性。例如，在足球比赛中踢点球，在棒球和垒球比赛中的投快球，在钢琴上弹奏快节奏的歌曲以及快速打字，都需要快速和准确的动作才能获得成功。对于这些技能来说，对速度和准确性的要求是由技能本身决定的。还有一些运动技能不要求速度，只要求肢体动作的准确性，例如，许多**手动瞄准技能（manual aiming skills）**要求将手移动到指定目标，但对速度没要求，执行者可以自行决定。这些技能包括：将钥匙插入钥匙孔、将笔放入笔筒、在电脑键盘上打字以及穿针引线。

对于这两类速度-准确性技能，即同时要求速度和准确性的动作与要求动作准确性但不要求速度的运动技能，我们通常可以观察到一种被称为**速度-准确性权衡**的现象。这意味着动作的准确性要求会影响动作速度，因此注重准确性会降低速度，而注重速度会降低准确性。因此，我们通常会用速度换准确性，或用准确性换速度。考虑一下这种权衡在上述两种技能的例子中发生的情况。对于足球比赛中的点球，当球员试图以最快的速度踢球时，通常的结果是踢不准。相反地，当球员注重准确性，试图将球踢到球门的某个区域时，结果往往是球踢得太慢而无法越过守门员。在将钥匙插入钥匙孔的例子中，钥匙移动过快通常会导致对不准钥匙孔，而注重准确性则会导致手的移动变慢。

**菲兹定律**

研究证据表明，速度-准确性权衡可以追溯到R. S. Woodworth于1899年发表的研究，R. S. Woodworth是运动控制研究的先驱者之一（Woodworth的实验和影响的综述，参见Elliott, Helsen, & Chua, 2001）。在接下来的半个世纪里，运动技能表现中速度-准确性权衡的现象已经变得如此普遍，以至于形成了一种数学定律，可以根据给定的准确性特征来预测动作速度。这个定律就是众所周知的菲兹定律，是由Paul Fitts（1954）基于自己的工作所描述的定律。菲兹定律已经成为了与人类表现相关的最有意义的“定律”之一。在科学领域中，*定律*指的是当包含某些变量时可以预测结果的情况。

**菲兹定律（Fitts’ Law）**可以为既需要速度又需要准确性的情况预测动作时间，在这种情况中，个体需要尽可能又快有准地移动到目标处。用于预测表现结果的变量是移动*距离*和*目标尺寸*。*根据菲兹定律，如果我们知道这两个变量的空间维度，就可以预测到达目标所需的动作时间。在数学术语中，菲兹定律将这种关系描述为
...
也就是说，动作时间等于以2为底，2倍移动距离除以目标宽度的对数。当移动距离增加时，动作时间会变长；当目标尺寸变小时，动作速度会降低以确保动作的准确性。换句话说，存在一种速度-准确性权衡。

菲兹指出，由于目标尺寸和移动距离之间的关系，等式中log2(2*D*/*W*)为速度-准确性技能提供了一个 **难度指数（index of difficulty, ID）**。难度指数越高，任务的困难程度越大。这是因为任务越困难，需要的动作时间越长。

图 7.1a 展示了几个交互敲击任务维度的例子来说明不同ID的特征。图 7.1b 展示了这些任务的近似动作时间，来说明当要求个体准确移动，即击中目标时，任务难度（ID）和动作速度之间的关系。

***菲兹定律适用于多种技能*** 菲兹的计算公式最初是建立在一个交互敲击任务上的，在这个任务中，受试者在指定时间内尽可能快地在两个目标之间做重复的往返运动，并被告知要注重准确性。

尽管菲兹定律是在一个实验室任务的基础上发展而来的，但值得注意的是，速度-准确性关系定律也同样适用于多种运动技能执行情境。例如，研究表明，当人们执行手动瞄准任务，如向目标投掷飞镖、够到或抓住不同大小的容器、弹钢琴、拿起钉子插入孔中，以及将屏幕上的光标移动到目标位置时，都表现出了菲兹定律所预测的动作时间特征。

El Lahib，Tekli和Issa（2018）的一项研究给出了菲兹定律适用于人机交互任务的例子。他们使用了一种常见的电脑任务，要求受试者使用指向设备去指屏幕上的物体，或用手指去触碰该物体。在对该指向任务的研究基础上，他们证明了菲兹定律“仍然是对指向任务进行数学建模的中心研究之一”，并且可以“很好地预测指向任务所需要的平均时间”（第16页）。

如果我们将菲兹定律应用于本节前面描述的体育运动技能中，应该就可以看出这一定律对运动指导和练习的影响。例如，假设要求一名足球运动员通过练习以尽可能快的速度将球踢进球门不同大小的区域，来练习点球得分。菲兹定律预测，踢向最大区域的球速度最快，而踢向最小区域的球速度最慢。在本书后面的部分（第6单元），我们将讨论帮助人们在这些情况下达到既快又准的练习条件。

***与速度-准确性权衡相关的开环和闭环运动控制过程*** 研究人员提出了几个假设来解释与速度-准确性权衡相关的运动控制过程。很多人详细阐述了Woodworth（1899）的原始假设，即在肢体快速移动到目标的过程中，有两个运动控制过程起作用，一个是*开环控制*过程，发生在运动的初始阶段并将肢体移动到目标附近，初始运动的速度、方向和准确性在无反馈的情况下由CNS控制。第二个是*闭环控制*过程，利用肢体相对于目标位置的视觉反馈来引导肢体“归位”，从而确保其准确地落在目标上。该假设是Woodworth在为动作开环或/和闭环阶段的运动控制活动提供详细描述时提出来的。然而，正如我们在第6章中所讨论的，当肢体接近目标时，一个人可否对动作进行纠正的主要决定因素在于可用时间量。这意味着，如果肢体在初始开环阶段的动作速度太快，那么当肢体接近目标时就不会有充足的时间根据视觉反馈来调整动作。


***视觉信息在速度-准确性权衡中的作用***当我们在速度-准确性技能，如将钥匙插入钥匙孔的表现中考虑感觉-知觉系统时就会发现，视觉在运动技能表现中起着主导作用。然而，值得注意的是，盲人身上也能看到菲兹定律的影响，这一点在El Lahib及其同事（2018）的研究中得到了证实。他们在实验中使用了振动触摸屏，让盲人受试者敲击该屏幕上的特定形状。


视觉在速度-准确性技能中的具体作用取决于肢体动作所处的阶段。尽管前一节中描述的假设为这些技能提出了两个不同的动作阶段，即开环和闭环阶段，但研究人员普遍认为，再增加一个包括技能执行所需的动作准备阶段，对于理解视觉在控制过程中的作用很重要。将钥匙插入钥匙孔这一手动瞄准技能是典型的速度-准确性技能，图7.2阐明了在该技能的执行过程中这个增加的阶段与其他两个动作阶段之间的关系，

*第一阶段*是*动作准备阶段（movement preparation）*，开始于一个人做出执行速度-准确性技能的决定。在这个阶段，人们使用视觉来确定技能执行环境中的调节条件。例如，对于将钥匙插入钥匙孔这一运动技能来说，调节条件包括钥匙的大小、形状和重量以及钥匙孔的大小、位置和方向。这些特征将明确肢体动作的方向和距离以及场景的准确性需求。此外，其他感觉信息，比如由触觉检测到的钥匙特征，也会一起传输到中枢神经系统中，为计划运动技能的开始和执行准备特定的动作特征。

动作准备阶段需要在速度和准确性中选择一种作为技能执行策略的基础。最近的fMRI研究证据表明，选择不同的策略会激活不同的大脑区域。（Vallesi, McIntosh, Crescentini, & Stuss, 2012）这一研究从一个有意思的层面证实了动作准备阶段中行为上的权衡会涉及到不同的脑区域。

*第二阶段*通常被称为*开始运行阶段（initial flight phase）*，包括朝目标方向移动的肢体动作，这个目标在我们的例子中就是钥匙孔。正如前面提到过的，这一动作是相对快速的（即弹射式的）并且不受感觉反馈的影响。尽管视觉在这一阶段所起的作用很小，但是它获取了肢体的位移和速度信息以及接触时间信息，这些信息在后面肢体接近目标时可以用来对动作进行修正。然而该阶段究竟是否需要对肢体运动的视觉，研究人员对此是一直存在分歧的（关于该问题的研究讨论，参见Elliott, Helsen, & Chua, 2001）。

*第三阶段*是*结束阶段（termination phase）*，从目标刚要被击中前开始，到击中时结束，在我们的例子中，击中目标就是指将钥匙插入钥匙孔中。对肢体和目标的视觉在该阶段很重要，可以将动作准确性信息传输到中枢神经系统，从而为实现击中目标的运动目标而对动作进行修正。

这里需要考虑的最后一点是，速度-准确性权衡的技能中*视觉与动作之间相互作用*的方式。值得注意的是，计划运动技能会影响视觉的作用方式。在刚刚讨论的手动瞄准任务的三个阶段中，将钥匙插入钥匙孔的意图会使视觉聚焦到钥匙孔上，从而在动作准备过程中提供动作需要遵守的关键调节条件的信息。该阶段的视觉信息也为钥匙向钥匙孔移动的初始运动轨迹和速度建立了基础。在结束阶段，将钥匙插入钥匙孔的运动目标会引导视觉去检测速度和准确性信息，从而进行精确的动作调整以实现运动目标。

**抓握**

**抓握**是一个用来描述涉及到*伸手抓取物体（reaching for and grasping of objects）*的运动技能的通用术语。研究证据表明，抓握运动技能包括三个不同的动作：*运送（transport），抓物（grasp）*和*物体操纵（object manipulation）*。物体操纵部分是指抓握运动技能的功能性目标，这就意味着，与指向和瞄准动作是不同的是，要理解抓握控制，就需要着重理解人们抓住物体后打算做什么。这部分的重要性在于，它会影响运送和抓住部分的运动学和动力学特征。例如，如果一个人想要拿起水杯喝水，这个运送和抓住的特征与只是想把杯子拿起来放到桌子的另一个位置上不同（关于该问题的运动控制影响的讨论，参见Newell & Cesari, 1999）。事实上，正是由于物体操纵部分与其他两个部分的关系，使得抓握运动技能必须区别于伸手取物和指向物体技能。

**抓握各组成部分之间的关系**

从运动控制的角度来看，抓握包括手臂将手运送到物体处，同时手部形成抓物所需要的特征。这里涉及了一个*运送成分与抓物成分*之间关系的问题。研究人员最初认为，这两个成分是相对独立的（如Jeannerod, 1981, 1984），但最近的研究证据表明，二者会根据任务需求*在时间上耦合并协同（即合作）交互作用*。这意味着，尽管伸手和抓物是两个独立的动作成分，但它们的作用方式是相互依赖的。

对这种耦合关系最有力的证据主要来自，当手向物体移动时对手指和拇指动作的分析。例如，Jakobson和Goodale（1991）在他们极具影响力的研究中表明，物体的大小以及与手之间的距离会影响伸手运送成分中手指和拇指之间距离（即抓握孔径）达到最大值的时间。有趣的是，他们发现*无论物体的大小和距离如何，最大抓握孔径和手闭合都发生在该运动技能中总动作持续时间的大约三分之二处*，这一发现也被其他研究人员证实（例如，Chieffi & Gentilucci, 1993; Hu, Osu, Okada, Goodale, & Kawato, 2005; Volcic & Domini, 2016）。研究人员提出了一个能够为任何手部运送动作预测出这种时间耦合关系的数学模型，从而为手部运送和抓握孔径之间的时间耦合关系提供了更确凿的证据支持。（Hu et al., 2005）与刚刚描述的研究相一致，该模型表明，最大抓握孔径出现在整个伸手动作总持续时间的大约三分之二处。

此外，研究证据表明，当物体在运送阶段发生突然的毫无征兆的移动（例如，Gentilucci, Chieffi, Scarpa, & Castiello, 1992），以及需要避开障碍物才能抵达物体时（例如，Saling, Alberts, Stelmach, & Bloedel, 1998），运送和抓物部分的运动学特征都需要进行调整。这些运动学上的变化再一次证明，抓握运动技能的伸手和抓物两个组成部分之间存在很强的*时间耦合*。因此，抓握是第5章中所描述理论的又一例证，即参与复杂运动技能的肌肉和关节作为一个协同合作的*协调结构（oordinative structure）*，使人们能够在各种情况下实现运动目标。


**视觉在抓握中的作用**

抓握是一种速度-准确性技能，因为需要准确的手部动作，并且动作速度会受到准确性需求的影响。因此，我们预期视觉应该以我们在前面关于速度-准确性技能中讨论的方式参与到抓握技能中。事实上，这里有一个例外，主要是在抓握运动技能的物体操纵部分。

与我们之前讨论的视觉在速度-准确性技能中的作用一致，视觉通过提供关于环境背景调节条件的信息来帮助准备抓握运动技能。这些信息与人们对物体的使用意图一起被传输到中枢神经系统，然后中枢神经系统就会对伸手动作中传送和抓物部分的时空特征进行大概的估计。一项要求受试者伸手抓取各种不同物体的研究，为关于物体属性的视觉信息在这种中枢神经系统规划的活动中的重要性提供了证据。（Verhagen, Dijkerman, Medendorp, & Toni, 2012）

接下来，视觉在手向物体移动的过程中也起着非常重要的作用，主要是检测手何时会接触到物体，从而可以适当地控制抓握孔径在时间上的变化。此外，视觉还提供关于握手相对于物体大小、形状和方向的空间特征的信息，从而可以适当地控制握手的空间特征。中枢神经系统将利用与这些动作特征有关的视觉反馈来进行必要的动作调整，以实现抓握的运动目标。事实上，研究表明，当一个人在运送的初始阶段无法看到物体时，抓物特征是会受到负面影响的（Fukui & Inui, 2006）。与我们前面讨论的速度-准确性技能不同，监视抓物这个动作本身的视觉可以补充触觉和本体感受反馈，以确保抓物动作在抓物和物体操纵阶段进行必要的调整。而且，正如Volcic和Domini（2016）所发现的那样，抓物动作会“顺利地自动依据当前的视觉反馈来进行调整”（第2174页），即使忽略视觉反馈并依赖触觉反馈可能会更有利。

正如第6章中所述，双目视觉提高了抓握运动技能的表现，尤其是在握手大小和力量的准备方面（Jackson, Newport, & Shaw, 2002）。此外，Brown，Haplert和Goodale（2005）的研究表明，由于中央视觉与周围视觉的作用不同，特别是它们分别与知觉视觉和动作视觉两个不同的视觉系统相关，因此当人们直接看向物体时，抓握技能的表现会得到提高。这种将注视点引导到物体的方式，使得两种视觉系统在抓握运动技能中发挥出最佳作用。
...
事实上，研究人员已经确定菲兹定律始终适用于实验室任务和日常生活活动的抓握动作。
...
例如，在 Bootsma, Marteniuk, MacKenzie, and Zaal (1994) 的一项实验中，根据菲兹定律的预测，移动距离和物体宽度会影响抓握过程中的移动时间。
...
A different approach to relating prehension actions to Fitts’ law was reported by Latash and Jaric (2002). 
 Latash and Jaric (2002) 报告了一种将抓握行为与菲兹定律联系起来的不同方法。
...
他们充分的理由是触碰、抓握和喝水需要准确度，这会影响手臂的运动速度以及相关肢体的协调性，这意味着菲兹定律应该适用。
...
在他们的实验中，研究人员开发了一种通过计算杯子大小和水位的比率从而计算出难度指数的方法，就像菲兹定律可以预测杯子最初移动到饮用者嘴的时间一样。
...
在刚才描述的双手协调任务中，两个瞄准任务中难度较大的(根据菲兹定律定义)会影响手臂执行难度较小的任务。
