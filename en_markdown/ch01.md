...
脚注1 有时你会看到术语*分析层面（level of analysis）*而非*研究层面（level of study）*。我们认为二者同义且可互换使用。
...
`		`b.To pick up a bowl of soup to move it from one location on a table to another location on the table
...
理解运动技能分类的依据可以帮助你确定，不同运动技能对学习者和执行者提出的要求有何不同，从而为客户建立恰当的教学环境。
...
(a)假设你是一名治疗师、教练或其他相关从业人员，描述一项你的客户会执行的具有尝试间变化的开放性运动技能。
